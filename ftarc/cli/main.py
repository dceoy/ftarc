...
def main() -> None:
    ...
    if args["init"]:
        ...
        gatk_or_picard = fetch_executable(
            "gatk", ignore_errors=(not args["bqsr"])
        ...
        if args["download"]:
            ...
            kwargs = {
                ...
                **{
                    c: fetch_executable(c)
                    for c in ["pigz", "pbzip2", "trim_galore", "cutadapt", "fastqc"]
                },
            }
            build_luigi_tasks(
                tasks=[
...
def _find_fq_paths(fq_path_prefix: str | os.PathLike[str]) -> list[str]:
    hits: list[Path] = sorted(
        ...
        and (
            o.name.endswith((".fq", ".fastq"))
            or o.name.endswith(
                tuple(
                    f".{a}.{b}" for a, b in product(["fq", "fastq"], ["", "gz", "bz2"])
                )
            )
        )
    )
